        ...
        <div id="kt_content_container" class="container-xxl">
            <div class="d-flex align-items-center justify-content-between mb-3">
                <ol class="breadcrumb breadcrumb-dot text-muted fs-6 fw-semibold">
                    ...
                    <li class="breadcrumb-item pe-3 text-primary">كل التخصصات</li>
                </ol>
                <a href="{{route('admin.courses-categories.create')}}" class="btn btn-success py-2 rounded fw-bolder"> اضافة جديد </a>
            </div>
                ...
                <div class="card-body fs-6 p-5 text-gray-700">
                    ...
                    <thead>
                        <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase">
                            ...
                            <th class="text-end min-w-75px">تاريخ الانشاء</th>
                    ...
                    <tbody class="fw-semibold text-gray-600">
                        ...
                        @foreach($categories as $category)
                                    <tr>
                                    <td>
                                <img src="{{$category->img}}" class='me-2 w-50px h-50px border rounded-circle shadow'  alt="">
                                <a href="{{route('admin.courses-categories.edit',['id'=>$category->id])}}" class="text-dark text-hover-primary"> {{$category->title}} </a>
                            </td>
                            <td >
                            ...
                            <td class="text-end pe-0">{{date('Y-m-d',strtotime($category->created_at))}}</td>
                            <td class="text-end">
                              @if($category->active == 1)  <span class="badge badge-light-success">فعال @else 
                                         ...
                                         <td class="text-end">
                                            <div class="btn-actions d-flex align-items-center justify-content-end">
                                                <label class="me-5 form-check form-switch form-check-custom form-check-solid d-inline-block">
                                                    <input class="form-check-input h-20px w-40px" type="checkbox" onclick="window.location='{{ route("admin.courses-categories.changeStatus",["id"=>$category->id]) }}'" id="flexSwitchCheckChecked" @if($category->active == 1) checked="" @endif/>
                                                </label>                                    
                                                <a  href="{{route('admin.courses-categories.edit',['id'=>$category->id])}}">
                                                ...
                                                <a   data-action="{{route('admin.courses-categories.delete',['id'=>$category->id])}}" data-name="{{$category->title}}"  class="rm" >
...
bootbox.dialog({
  message: "<form class='form-inline add-to-ban' method='GET'><div class='form-group'>هل تريد تأكيد حذف التخصص   "+current_object.attr('data-name')+" ?</div></form>",
  title: "حذف",
  buttons: {
    success: {
      label: "حذف",
