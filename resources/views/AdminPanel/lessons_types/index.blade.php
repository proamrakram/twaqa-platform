        ...
        <div id="kt_content_container" class="container-xxl">
            ...
            <div class="d-flex align-items-center justify-content-between mb-3">
                <ol class="breadcrumb breadcrumb-dot text-muted fs-6 fw-semibold">
                    ...
                    <li class="breadcrumb-item pe-3"><a href="{{route('admin.lessons-types.index')}}" class="pe-3 text-muted">نوع الحلقة</a></li>
                    <li class="breadcrumb-item pe-3 text-primary">اضافة جديد</li>
                </ol>
            </div>        
            <div class="row">
                <div class="col-md-4">
                <form  method="post" action="{{ route('admin.lessons-types.store') }}" enctype="multipart/form-data">
                    @csrf

                        <div class="mb-3">
                            <label for="nameTypeCourse" class="form-label fw-bolder d-block"> الاسم  </label>
                            <input name="title" type="text" id="nameTypeCourse" class="form-control @error('title') is-invalid @enderror" placeholder=""/> 
                            @error('title')
                        ...
                        <div class="mb-3">
                            <label for="descTypeCourse" class="form-label fw-bolder d-block"> الوصف  </label>
                            <textarea name="description"  type="text" id="descTypeCourse" class="form-control @error('description') is-invalid @enderror" placeholder=""></textarea>
                            @error('description')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                         ...
                                         <td class="text-end">
                                            <div class="btn-actions d-flex align-items-center justify-content-end">
                                                <label class="me-5 form-check form-switch form-check-custom form-check-solid d-inline-block">
                                                    <input class="form-check-input h-20px w-40px" type="checkbox" onclick="window.location='{{ route("admin.lessons-types.changeStatus",["id"=>$type->id]) }}'" id="flexSwitchCheckChecked" @if($type->active == 1) checked="" @endif/>
                                                </label>                                    
                                                <a  href="{{route('admin.lessons-types.edit',['id'=>$type->id])}}">
                                                ...
                                                <a  data-action="{{route('admin.lessons-types.delete',['id'=>$type->id])}}"  data-name="{{$type->title}}" class="rm">
...
var KTDatatablesExample = function () {
    ...
    // Hook export buttons
    var exportButtons = () => {
        const documentTitle = 'كل أنواع الحلقات في أكاديمية تواقة';
...
bootbox.dialog({
  message: "<form class='form-inline add-to-ban' method='GET'><div class='form-group'>هل تريد تأكيد حذف نوع الحلقة   "+current_object.attr('data-name')+" ?</div></form>",
  title: "حذف",
  buttons: {
    success: {
      label: "حذف",
