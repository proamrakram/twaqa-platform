            ...
            <div class="card">
                ...
                <form  method="post" action="{{ route('admin.notifications.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                            ...
                            <input type="text" name="title" value="{{old('title')}}" class="form-control form-control-solid @error('title') is-invalid @enderror" >
                            @error('title')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea  name="description" class="form-control form-control-solid @error('description') is-invalid @enderror"  >{{old('description')}}</textarea>
                            @error('description')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                        ...
                        <div class="mb-3">
                            ...
                            <select  name="to"    id="type-dropdown"  class="form-select form-select-solid @error('to') is-invalid @enderror" data-control="select2" data-dropdown-css-class="w-200px" data-placeholder="اختر .." data-hide-search="true">
                                <option></option>
                                <option value="teacher" @if(old('to')=='teacher') selected @endif> معلم  </option>
                                <option value="student"   @if(old('to')=='student') selected @endif>طالب </option>
                            </select>
                            @error('to')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
...
$.each(result.users,function(key,value){
    $("#users-dropdown").append('<option value="'+value.id+'">'+value.name+'</option>');
}); 
}
});
});   
