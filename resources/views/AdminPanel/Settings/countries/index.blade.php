        ...
        <div id="kt_content_container" class="container-xxl">
            <div class="d-flex align-items-center justify-content-between mb-3">
                <ol class="breadcrumb breadcrumb-dot text-muted fs-6 fw-semibold">
                    ...
                    <li class="breadcrumb-item pe-3 text-primary">كل الدول</li>
                </ol>
                <a href="{{route('admin.countries.create')}}" class="btn btn-success py-2 rounded fw-bolder"> اضافة جديد </a>
            </div>
                ...
                <div class="card-body fs-6 p-5 text-gray-700">
                    ...
                    <tbody class="fw-semibold text-gray-600">
                        ...
                        @foreach($countries as $country)
                                    <tr>
                                    <td>
                                <img src="{{$country->country_flag}}" class='me-2 w-50px h-50px border rounded-circle shadow'  alt="">
                                <a href="{{route('admin.countries.edit',['id'=>$country->id])}}" class="text-dark text-hover-primary"> {{$country->country_name}} </a>
                            </td>
                            <td class="text-end">
                              @if($country->status == 'active')  <span class="badge badge-light-success">فعال @else 
                                         ...
                                         <td class="text-end">
                                            <div class="btn-actions d-flex align-items-center justify-content-end">
                                                <label class="me-5 form-check form-switch form-check-custom form-check-solid d-inline-block">
                                                    <input class="form-check-input h-20px w-40px" type="checkbox" onclick="window.location='{{ route("admin.countries.changeStatus",["id"=>$country->id]) }}'" id="flexSwitchCheckChecked" @if($country->status == 'active') checked="" @endif/>
                                                </label>                                    
                                                ...
                                                <a   data-action="{{route('admin.countries.delete',['id'=>$country->id])}}" data-name="{{$country->country_name}}"  class="rm" >
...
var KTDatatablesExample = function () {
    ...
    // Hook export buttons
    var exportButtons = () => {
        const documentTitle = 'كل الدول في أكاديمية تواقة';
...
bootbox.dialog({
  message: "<form class='form-inline add-to-ban' method='GET'><div class='form-group'>هل تريد تأكيد حذف الدولة   "+current_object.attr('data-name')+" ?</div></form>",
  title: "حذف",
  buttons: {
    success: {
      label: "حذف",
