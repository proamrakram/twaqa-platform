        ...
        <div id="kt_content_container" class="container-xxl">
            <div class="d-flex align-items-center justify-content-between mb-3">
                <ol class="breadcrumb breadcrumb-dot text-muted fs-6 fw-semibold">
                    ...
                    <li class="breadcrumb-item pe-3 text-primary">كل المشرفين</li>
                </ol>
                <a href="{{route('admin.supervisors.create')}}" class="btn btn-success py-2 rounded fw-bolder"> اضافة جديد </a>
            </div>
            ...
            <div class="card">
                <div class="card-body fs-6 p-5 text-gray-700">
                    ...
                    <table class="table align-middle border rounded table-row-dashed fs-6 g-5" id="kt_datatable_supervisors">
                    <thead>
                        <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase">
                            <th class="min-w-100px">الاسم </th>
                            <th class="min-w-100px">البريد الإلكتروني</th>
                            <th class="min-w-100px">العنوان</th>
                            <th class="min-w-100px">الوظيفة الحالية</th>
                            <th class="text-end min-w-75px">تاريخ الإنضمام</th>
                    ...
                    <tbody class="fw-semibold text-gray-600">
                        ...
                        <tr>
                            <td>
                            <a href="{{route('admin.supervisors.edit',['id'=>$user->id])}}" class="text-dark text-hover-primary">{{$user->full_name}}</a>
                            </td>
                            <td>
                            {{$user->email}}
                            </td>
                            ...
                            <td >{{$user->position}}</td>
                            <td class="text-end pe-0">{{date('Y-m-d',strtotime($user->registered_at))}}</td>
                            <td class="text-end">
                            @if($user->active == 1)  <span class="badge badge-light-success">فعال @else 
                            ...
                            <td class="text-end">
                                            <div class="btn-actions d-flex align-items-center justify-content-end">
                                                <label class="me-5 form-check form-switch form-check-custom form-check-solid d-inline-block">
                                                    <input class="form-check-input h-20px w-40px" type="checkbox" onclick="window.location='{{ route("admin.supervisors.changeStatus",["id"=>$user->id]) }}'" id="flexSwitchCheckChecked" @if($user->active == 1) checked="" @endif/>
                                                </label>                                    
                                                <a  href="{{route('admin.supervisors.edit',['id'=>$user->id])}}">
                                                ...
                                                <a  data-action="{{route('admin.supervisors.delete',['id'=>$user->id])}}"    data-name="{{$user->full_name}}" class="rm">
...
var KTDatatablesExample = function () {
    ...
    return {
        init: function () {
            table = document.querySelector('#kt_datatable_supervisors');
...
bootbox.dialog({
  message: "<form class='form-inline add-to-ban' method='GET'><div class='form-group'>هل تريد تأكيد حذف المشرف  "+current_object.attr('data-name')+" ?</div></form>",
  title: "حذف",
  buttons: {
    success: {
      label: "حذف",
