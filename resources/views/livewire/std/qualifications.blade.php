<div>
    <form action="#" class="box-about-teacher bg-white border rounded-3 p-4 mb-3 update-form" wire:ignore.self>
        <div class="d-flex border-bottom pb-3 mb-3" wire:ignore.self>

            <div class="icon ms-3" wire:ignore.self>
            ...
            <div class="text d-flex justify-content-between w-100 repeater-edit" wire:ignore.self>

                <div class="inputs w-75" wire:ignore.self>

                    <input class="h5 clr-royal-blue mb-1 form-input {{ $edit }}"
                        @if (!$edit) disabled @endif wire:model='qualification_degree'>
                    @error('qualification_degree')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror

                    <input class="form-input {{ $edit }}" @if (!$edit) disabled @endif
                        wire:model='university'>
                    @error('university')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                    <select wire:model='country_id' @if (!$edit) disabled @endif
                        class='form-control mb-2 border p-1 {{ $edit }}' placeholder='الدولة'
                    ...
                    <input class="fw-bold gray-clr font-number form-input {{ $edit }}"
                        @if (!$edit) disabled @endif wire:model='year'>
                    @error('year')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>

                <div class="btns d-flex align-items-start" wire:ignore.self>

                    @if ($edit)
                        <button type="button" class='btn-green ms-1 text-white' wire:click='update'>
                            حفظ </button>
                    @endif

                    <button type="button" class='delete ms-1' wire:click='delete'>
                        <i class="fa-solid fa-trash"></i>
                    </button>

                    <div class="icon icon-edit-repeater" wire:click='edit' wire:ignore.self>
