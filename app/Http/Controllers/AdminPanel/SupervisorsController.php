...
class SupervisorsController extends Controller
{
  ...
  {
    $users = User::where('is_delete',0)->where('user_type','supervisor')->get();
    return view('AdminPanel.supervisors.index')->with('users', $users);
  }
  ...
  public function create()
  {
    ...
    return view('AdminPanel.supervisors.create')->with('categories', $categories)->with('countries',$countries);
  }
...
if(is_null($request->avatar)){
  ...
  public function edit($id)
  {
    ...
    if(is_null($user) || $user->is_delete == 1  || $user->user_type != 'supervisor') {
      $this->massage('error','Not found', 'غير موجود', 'No encontrado');
      return redirect()->back();
    }
    $countries = Country::get();
    $categories = CourseCategory::where('is_delete',0)->where('active',1)->get();
    return view('AdminPanel.supervisors.edit')->with('user', $user)->with('countries',$countries)->with('categories', $categories);
  }
